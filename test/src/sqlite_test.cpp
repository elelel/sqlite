...
CREATE TABLE `table1` (
  ...
  WHEN("Inserting a record") {
    ...
    WHEN("unwrapped") {
      WHEN("as lvalue params tuple") {
        auto params = sqlite::make_params(1, 2);
        q_insert.params.bind(params);
      }
      WHEN("Binding by index") {
        WHEN("One by one") {
          q_insert.params.bind_value(1, 1);
          q_insert.params.bind_value(1, 2);
        }
        WHEN("Variadic") {
          q_insert.params.bind_values(1, 2);
        }
      }
    }
  }
